...
- **ActivityDraftService**: Manages the draft lifecycle and parallel markdown generation
...
- **Übung & Lösungsblatt**: Generates paired exercise and solution sheets in a single LLM call. When a vision model is configured (`LLM_MODEL_VISUAL`), PDF page images are sent alongside the prompt.
- **Tafelbild**: Generates a board-diagram image using the optional Azure OpenAI image model.

`[[IMAGE_PLACEHOLDER: ...]]` markers emitted by the exercise LLM are replaced with images generated by the optional Azure OpenAI image model (`LLMService.generateImageMarkdown`). If the image model is not configured, placeholders are preserved in the markdown output.
