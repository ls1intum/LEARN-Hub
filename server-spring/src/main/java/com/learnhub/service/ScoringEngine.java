...
import com.learnhub.dto.response.CategoryScoreResponse;
import com.learnhub.dto.response.ScoreResponse;
...
public class ScoringEngine {
    ...
    private CategoryScoreResponse scoreAgeAppropriateness(Activity activity, SearchCriteria criteria) {
        ...
        if (activity.getAgeMin() <= targetAge && targetAge <= activity.getAgeMax()) {
            ...
            int distance = targetAge < activity.getAgeMin() ? 
                activity.getAgeMin() - targetAge : targetAge - activity.getAgeMax();
    ...
    private CategoryScoreResponse scoreTopicRelevance(Activity activity, SearchCriteria criteria) {
        ...
        List<ActivityTopic> preferredTopics = criteria.getPreferredTopics();
        
        if (preferredTopics == null || preferredTopics.isEmpty()) {
            return createCategoryScore(category, 0);
        }
        
        List<ActivityTopic> activityTopics = activity.getTopics();
        ...
        double rawScore = preferredSet.isEmpty() ? 0.0 : 
            ((double)intersection.size() / preferredSet.size()) * 100.0;
    ...
    private CategoryScoreResponse scoreDurationFit(List<Activity> activities, SearchCriteria criteria) {
        ...
        for (int i = 0; i < activities.size(); i++) {
            ...
            if (activity.getDurationMinMinutes() != null) {
                int durationMax = activity.getDurationMaxMinutes() != null ? 
                    activity.getDurationMaxMinutes() : activity.getDurationMinMinutes();
    ...
    private CategoryScoreResponse scoreSeriesCohesion(List<Activity> activities) {
        ...
        for (int i = 0; i < activities.size() - 1; i++) {
            Set<String> currentTopics = activities.get(i).getTopics() != null ?
                activities.get(i).getTopics().stream().map(t -> t.name().toLowerCase())
                    .collect(Collectors.toSet()) : new HashSet<>();
            Set<String> nextTopics = activities.get(i + 1).getTopics() != null ?
                activities.get(i + 1).getTopics().stream().map(t -> t.name().toLowerCase())
                    .collect(Collectors.toSet()) : new HashSet<>();
    ...
    public static class SearchCriteria {
        ...
        private List<ActivityTopic> preferredTopics;
        ...
        public Integer getTargetAge() { return targetAge; }
        public void setTargetAge(Integer targetAge) { this.targetAge = targetAge; }
        
        public List<ActivityFormat> getFormats() { return formats; }
        public void setFormats(List<ActivityFormat> formats) { this.formats = formats; }
        
        public List<BloomLevel> getBloomLevels() { return bloomLevels; }
        public void setBloomLevels(List<BloomLevel> bloomLevels) { this.bloomLevels = bloomLevels; }
        ...
        public List<ActivityResource> getAvailableResources() { return availableResources; }
        public void setAvailableResources(List<ActivityResource> availableResources) { 
            this.availableResources = availableResources; 
        }
        
        public List<ActivityTopic> getPreferredTopics() { return preferredTopics; }
        public void setPreferredTopics(List<ActivityTopic> preferredTopics) { 
