...
import com.learnhub.service.ScoringEngine.SearchCriteria;
...
public class RecommendationService {
    ...
    private SearchCriteria convertCriteria(Map<String, Object> criteriaMap) {
        ...
        if (criteriaMap.containsKey("preferred_topics")) {
            criteria.setPreferredTopics(toEnumList(criteriaMap.get("preferred_topics"), ActivityTopic.class));
        }
    ...
    private boolean matchesCriteria(Activity activity, SearchCriteria criteria) {
        ...
        if (criteria.getAvailableResources() != null && !criteria.getAvailableResources().isEmpty()) {
            List<ActivityResource> activityResources = activity.getResourcesNeeded();
            if (activityResources != null && !activityResources.isEmpty()) {
                boolean hasAllResources = activityResources.stream()
                    .allMatch(resource -> criteria.getAvailableResources().contains(resource));
                if (!hasAllResources) {
                    return false;
                }
            }
        ...
        if (criteria.getPreferredTopics() != null && !criteria.getPreferredTopics().isEmpty()) {
            ...
            boolean hasAnyTopic = activityTopics.stream()
                .anyMatch(topic -> criteria.getPreferredTopics().contains(topic));
            if (!hasAnyTopic) {
                return false;
            }
        }
    ...
    private int calculateBreakDuration(Activity current, Activity next) {
        // Simple heuristic: 5 minutes for topic changes, 10 minutes for energy level changes
        int breakDuration = 0;
        
        // Check topic change
        Set<ActivityTopic> currentTopics = new HashSet<>(current.getTopics());
        Set<ActivityTopic> nextTopics = new HashSet<>(next.getTopics());
